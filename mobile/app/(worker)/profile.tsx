...
export default function WorkerProfileScreen() {
  ...
  const handleLogout = () => {
    Alert.alert("Logout", "Are you sure?", [
      ...
      { text: "Logout", style: "destructive", onPress: () => { logout(); router.replace("/auth/role-select"); } },
    ]);
  };
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statBox}>
            <Text style={styles.statVal}>{workerProfile?.completed_jobs || 0}</Text>
            <Text style={styles.statLbl}>Jobs Done</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={styles.statVal}>⭐ {(workerProfile?.rating || 0).toFixed(1)}</Text>
            <Text style={styles.statLbl}>Rating</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={styles.statVal}>{workerProfile?.total_reviews || 0}</Text>
          ...
          <View style={styles.cardRow}>
            <View>
              ...
              <Text style={styles.cardSubtitle}>
                {workerProfile?.is_available ? "Employers can see and hire you" : "You're currently not visible"}
              </Text>
            </View>
            <Switch
              value={workerProfile?.is_available || false}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>{skill?.icon} {skill?.label || "—"}</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>Experience</Text>
            <Text style={styles.detailValue}>{workerProfile?.experience_years || 0} years</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>Daily Wage</Text>
            <Text style={styles.detailValue}>₹{(workerProfile?.daily_wage || 0).toLocaleString("en-IN")}</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>City</Text>
            <Text style={styles.detailValue}>{workerProfile?.city || "—"}</Text>
          </View>
          {workerProfile?.bio && (
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Bio</Text>
              <Text style={[styles.detailValue, { flex: 1, textAlign: "right" }]}>{workerProfile.bio}</Text>
            </View>
          )}
        </View>

        <View style={styles.actions}>
          <Button title="Notifications" onPress={() => router.push("/(worker)/notifications")} variant="outline" style={{ marginBottom: 10 }} />
