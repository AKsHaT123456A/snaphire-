      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statBox}>
            <Text style={styles.statVal}>{employerProfile?.total_hires || 0}</Text>
            <Text style={styles.statLbl}>Total Hires</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={styles.statVal}>⭐ {(employerProfile?.rating || 0).toFixed(1)}</Text>
            <Text style={styles.statLbl}>Rating</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={styles.statVal}>{employerProfile?.total_reviews || 0}</Text>
        ...
        <View style={styles.card}>
          ...
          {employerProfile?.company_name && (
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{employerProfile.company_name}</Text>
            </View>
          )}
          {employerProfile?.industry && (
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Industry</Text>
              <Text style={styles.detailValue}>{employerProfile.industry}</Text>
            </View>
          )}
          {employerProfile?.city && (
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>City</Text>
              <Text style={styles.detailValue}>{employerProfile.city}</Text>
            </View>
          )}
        </View>

        <View style={styles.actions}>
          <Button title="Notifications" onPress={() => router.push("/(employer)/notifications")} variant="outline" style={{ marginBottom: 10 }} />
