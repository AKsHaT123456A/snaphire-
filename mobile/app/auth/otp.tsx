...
import {
  View, Text, StyleSheet, SafeAreaView, TextInput, TouchableOpacity, Alert,
} from "react-native";
...
export default function OTPScreen() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (!profile_complete) {
        router.replace(userRole === "worker" ? "/auth/worker-setup" : "/auth/employer-setup");
      } else {
        router.replace(userRole === "worker" ? "/(worker)/home" : "/(employer)/home");
      }
    } catch (e: any) {
...
const styles = StyleSheet.create({
  ...
  hint: {
    ...
    borderLeftWidth: 3, borderLeftColor: Colors.primary, marginBottom: 24,
  },
