...
export default function EmployerWorkersScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {workers.length === 0 ? (
        <EmptyState icon="👷" title="No workers found" subtitle="No available workers match your filters. Try a different skill or expand your search area." ctaLabel="Clear Filters" onCta={() => setSkillFilter(null)} />
      ) : (
        <FlashList
          data={workers}
          ...
          renderItem={({ item }) => (
            <View style={{ paddingHorizontal: 20 }}>
              <WorkerCard worker={item} onPress={() => navigation.navigate("WorkerProfileDetail", { id: item.id })} onHire={() => navigation.navigate("WorkerProfileDetail", { id: item.id })} />
            </View>
          )}
