    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.container}>
        ...
        <View style={styles.cards}>
          <TouchableOpacity style={[styles.roleCard, selected === "worker" && styles.roleCardSelected]} onPress={() => setSelected("worker")} activeOpacity={0.85}>
            <Text style={styles.roleIcon}>👷</Text>
            <Text style={styles.roleTitle}>Worker</Text>
            <Text style={styles.roleDesc}>Find daily wage jobs near you. Mason, Carpenter, Electrician and more.</Text>
            {selected === "worker" && <View style={styles.checkmark}><Text style={styles.checkmarkText}>✓</Text></View>}
          </TouchableOpacity>

          <TouchableOpacity style={[styles.roleCard, selected === "employer" && styles.roleCardSelected]} onPress={() => setSelected("employer")} activeOpacity={0.85}>
            <Text style={styles.roleIcon}>🏗️</Text>
            <Text style={styles.roleTitle}>Employer</Text>
            <Text style={styles.roleDesc}>Post jobs and hire skilled workers nearby. Fast, verified, reliable.</Text>
            {selected === "employer" && <View style={styles.checkmark}><Text style={styles.checkmarkText}>✓</Text></View>}
          </TouchableOpacity>
        </View>

        <View style={styles.footer}>
          <Button title="Continue" onPress={() => navigation.navigate("Phone", { role: selected! })} disabled={!selected} size="lg" />
...
const styles = StyleSheet.create({
  ...
  roleCard: { backgroundColor: Colors.white, borderRadius: 20, padding: 24, borderWidth: 2, borderColor: Colors.border, position: "relative" },
  roleCardSelected: { borderColor: Colors.primary, backgroundColor: "#FFF8F0" },
  ...
  checkmark: { position: "absolute", top: 16, right: 16, width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.primary, alignItems: "center", justifyContent: "center" },
