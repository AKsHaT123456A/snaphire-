  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={[styles.urgentBtn, urgentOnly && styles.urgentBtnActive]} onPress={() => setUrgentOnly(!urgentOnly)}>
          <Text style={[styles.urgentText, urgentOnly && styles.urgentTextActive]}>🔥 Urgent</Text>
        </TouchableOpacity>
      </View>

      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={styles.filterContent}>
        <TouchableOpacity style={[styles.filterChip, !skillFilter && styles.filterChipActive]} onPress={() => setSkillFilter(null)}>
          <Text style={[styles.filterText, !skillFilter && styles.filterTextActive]}>All</Text>
        </TouchableOpacity>
        {SKILL_CATEGORIES.map((s) => (
          <TouchableOpacity key={s.value} style={[styles.filterChip, skillFilter === s.value && styles.filterChipActive]} onPress={() => setSkillFilter(skillFilter === s.value ? null : s.value)}>
      ...
      {jobs.length === 0 ? (
        <EmptyState icon="💼" title="No jobs found" subtitle="No jobs match your filters right now. Try expanding your search or check back later." ctaLabel="Clear Filters" onCta={() => { setSkillFilter(null); setUrgentOnly(false); }} />
      ) : (
        <FlashList
          data={jobs}
...
const styles = StyleSheet.create({
  ...
  urgentBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: Colors.border, backgroundColor: Colors.white },
  ...
  filterChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: Colors.border, backgroundColor: Colors.white },
