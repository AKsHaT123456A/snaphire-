...
export default function WorkerWalletScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const [wRes, tRes] = await Promise.all([api.get("/wallet/"), api.get("/wallet/transactions")]);
      setWallet(wRes.data); setTransactions(tRes.data);
    } catch {}
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} tintColor={Colors.primary} />}>
        <View style={styles.header}><Text style={styles.title}>My Wallet</Text></View>

        <View style={styles.balanceCard}>
          <Text style={styles.balanceLabel}>Total Earned</Text>
          <Text style={styles.balanceAmount}>₹{(wallet?.total_earned || 0).toLocaleString("en-IN")}</Text>
          <View style={styles.balanceRow}>
            <View style={styles.balanceStat}><Text style={styles.balanceStatVal}>₹{(wallet?.balance || 0).toLocaleString("en-IN")}</Text><Text style={styles.balanceStatLbl}>Available</Text></View>
            <View style={styles.balanceDivider} />
            <View style={styles.balanceStat}><Text style={styles.balanceStatVal}>₹{((wallet?.total_earned || 0) - (wallet?.balance || 0)).toLocaleString("en-IN")}</Text><Text style={styles.balanceStatLbl}>Withdrawn</Text></View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {transactions.length === 0 ? (
            <EmptyState icon="💸" title="No payments yet" subtitle="Complete your first job to see your earnings here." />
          ) : (
...
const styles = StyleSheet.create({
  ...
  balanceCard: { marginHorizontal: 20, marginBottom: 24, backgroundColor: Colors.primary, borderRadius: 24, padding: 24 },
  ...
  txnCard: { flexDirection: "row", alignItems: "center", backgroundColor: Colors.white, borderRadius: 14, padding: 14, marginBottom: 8 },
